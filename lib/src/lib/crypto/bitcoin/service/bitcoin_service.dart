...
import 'package:bit_saifu/src/lib/crypto/bitcoin/service/bitcoin_service_base.dart';
...
class NewworkBitcoinService implements BitcoinServiceBase {
  final CryptoNetwork network;

  final BitcoinRepository repository;
  final KeyPairGenerator keyPairGenerator;
  final BitcoinAddressFactory addressFactory;

  NewworkBitcoinService({
    required this.network,
    required this.repository,
    required this.keyPairGenerator,
    required this.addressFactory,
  });
  ...
  Future<BitcoinEntity> generateAddress(
      {required BitcoinAddressType type}) async {
    final keyPair = keyPairGenerator.generate();
    final publicKey = keyPair.publicKey;

    final address =
        addressFactory.createAddress(publicKey: publicKey, type: type);

    final bitcoinEntity = BitcoinEntity(
  ...
  Future<List<BitcoinUtxo>> getUtxos(
      {required BitcoinEntity bitcoinEntity}) async {
    final address = bitcoinEntity.address;
    final utxos = await repository.getUtxos(address);
    return utxos
        .map((utxo) => BitcoinUtxo(
              txid: utxo.txid,
              vout: utxo.vout,
              value: utxo.value,
